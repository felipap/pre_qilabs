...
onGetPosts = function (posts) {
	db.signers.getAll(function (users) {
		users.forEach(function (user) {
			
			var tags = _.union.apply(null, _.pluck(_.filter(posts, function (post) {
					return new Date(post.date) > new Date(user.last_update);
				}), 'tags'));

			if (tags.length)
				notifyUser(user, tags)
			
			user.last_update = new Date(0);
			db.signers.updateUser(user);
		});
	});
}
